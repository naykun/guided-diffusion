...
def do_run():
    ...
    for prompt in prompts:
        ...
        target_embeds.append(clip_model.encode_text(clip.tokenize(prompt,truncate=True).to(device)).float())
    ...
    for i in range(args.num_batches):
        ...
        for k, image in enumerate(origin_images):
            filename = f'origin_{i * args.batch_size + k:05}.png'
