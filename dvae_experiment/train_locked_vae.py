...
train_group.add_argument('--lr_decay_rate', type = float, default = 0.985, help = 'learning rate decay')
...
model_group.add_argument('--kl_loss_weight', type = float, default = 0.0, help = 'KL loss weight')
...
loaded_obj = torch.load('classifier-final.pt')
...
vae.load_state_dict(vae_weights, strict=False) # load encoder and codebook only

# only train decoder
for name, p in vae.named_parameters():
    if 'encoder' in name or 'codebook' in name:
