...
parser.add_argument('--model_path', type=str, default = './models/model-ru-latest.pt',
...
def do_run():
    ...
    if args.input.endswith('npy'):
        ...
        b, n = img_seq.shape
        one_hot_indices = torch.nn.functional.one_hot(img_seq, num_classes=vae.num_tokens).float()
        z = (one_hot_indices @ vae.model.quantize.embed.weight)

        embeds = rearrange(z, 'b (h w) c -> b c h w', h = int(sqrt(n)))
    else:
